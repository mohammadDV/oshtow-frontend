  ...
  return (
    <>
      ...
      <div className="grid lg:grid-cols-3 gap-5 mt-6 px-6 md:px-0">
        {plansData
          ?.filter((plan) => plan.priod === selectedTab)
          .map((item) => (
            <div key={item.id} className="px-5 py-7 rounded-3xl bg-white">
              ...
              <div className="flex flex-col gap-2.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <p className=" text-text font-normal">
                    {selectedTab === "monthly"
                      ? tPages("profile.plans.monthlyPlanCredit")
                      : tPages("profile.plans.yearlyPlanCredit")}
                  </p>
                </div>
                <div className="flex items-center gap-1.5">
                  <Icon
                  ...
                  {item.project_count == "0" ? (
                    <p className=" text-text font-normal">
                      {tPages("profile.plans.unlimitAds")}
                    </p>
                  ) : (
                    <p className=" text-text font-normal">
                      {item.project_count}{" "}
                      {selectedTab === "monthly"
                        ? tPages("profile.plans.adsInMonth")
                        : tPages("profile.plans.adsInYear")}
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  {item.claim_count == "0" ? (
                    <p className=" text-text font-normal">
                      {tPages("profile.plans.unlimitClaim")}
                    </p>
                  ) : (
                    <p className=" text-text font-normal">
                      {item.claim_count}{" "}
                      {selectedTab === "monthly"
                        ? tPages("profile.plans.claimInMonth")
                        : tPages("profile.plans.claimInYear")}
                    </p>
                  )}
                </div>
              </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between my-4">
            <p className="text-text text-sm font-medium">
              {selectedPlan?.title} /{" "}
              {selectedPlan?.priod === "monthly"
                ? tPages("profile.plans.monthly")
                : tPages("profile.plans.yearly")}
            </p>
            {selectedPlan && (
              <p className="text-text text-sm font-medium">
                {putCommas(parseFloat(selectedPlan.amount))}{" "}
                {tCommon("unit.toman")}
              </p>
            )}
          </div>
          ...
          <div className="mb-3">
            ...
            <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
              <div className="flex items-center gap-1.5">
                ...
                <label
                  htmlFor="wallet"
                  className="text-sm text-text font-normal cursor-pointer flex-1"
                >
                  {tPages("profile.plans.walletPayment")} (
                  {tPages("profile.plans.walletBalance")}:
                  {putCommas(walletData?.data?.available_balance)}{" "}
                  {tCommon("unit.toman")}) -
                  <Link href={"/profile/wallet"} className="text-primary mr-1">
                    {tPages("profile.plans.chargeWallet")}
                  </Link>
                </label>
              </div>
              <div className="flex items-center gap-1.5">
